...
CREATE TABLE IF NOT EXISTS public.reload_loop_events (
...
DROP POLICY IF EXISTS "Anyone can log reload-loop events" ON public.reload_loop_events;
CREATE POLICY "Anyone can log reload-loop events"
  ON public.reload_loop_events FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Admins can view reload-loop events" ON public.reload_loop_events;
CREATE POLICY "Admins can view reload-loop events"
  ON public.reload_loop_events FOR SELECT
  TO authenticated
  USING (public.is_admin(auth.uid()));

CREATE INDEX IF NOT EXISTS reload_loop_events_created_at_idx ON public.reload_loop_events (created_at DESC);
CREATE INDEX IF NOT EXISTS reload_loop_events_event_idx ON public.reload_loop_events (event);
