...
import { persistWithRetry, telegramRequest } from "../_shared/telegram.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!marked) {
      return jsonResponse({ error: "Post dispatch could not be started" }, 409);
    }
    ...
    if (!recorded) {
      console.error("Failed to record posted status after retries");
      return jsonResponse({
        success: true,
