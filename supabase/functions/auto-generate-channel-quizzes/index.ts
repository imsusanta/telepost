...
import { normalizeTelegramQuizQuestion } from "../_shared/telegram-quiz.ts";
...
serve(async (req) => {
  ...
  const classified = classifyBearer({ authorizationHeader: req.headers.get("Authorization"), cronSecretHeader: req.headers.get("x-cron-secret"), cronSecret, serviceRoleKey });
  if (classified === "missing") return new Response(JSON.stringify({ error: "Unauthorized" }), { status: 401, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  ...
  try {
    ...
    if (!isInternal && specificChannelId) {
      ...
      const decision = authorizeOwnedRecord({ classified, callerUserId, ownerUserId: ownedChannel?.user_id, recordExists: Boolean(ownedChannel) });
      if (decision === "not_found") return new Response(JSON.stringify({ error: "Channel not found" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      if (decision === "forbidden" || decision === "unauthorized") return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    if (!isInternal) {
      if (!callerUserId) return new Response(JSON.stringify({ error: "Forbidden" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    for (const channel of channels as Channel[]) {
      try {
        ...
        const { data: topics } = await supabase.from("knowledge_base_topics").select("topic, subject, description, language, ai_instructions").eq("channel_id", channel.id).eq("user_id", channel.user_id).eq("is_active", true).order("created_at", { ascending: false }).limit(50);
        const knowledgeBase = (topics || []).map((item: any) => `Topic: ${item.topic_name || item.topic}\nSubject: ${item.subject || ""}\nDescription: ${item.description || ""}\nLanguage: ${item.language || "bn"}\nInstructions: ${item.ai_instructions || item.prompt_context || ""}`).join("\n\n---\n\n").substring(0, 12000);
        const topic = channel.settings.default_subject || topics?.[0]?.topic || topics?.[0]?.topic_name || channel.name;
        ...
        const normalizedQuestions = quiz.questions.map((question: any) => normalizeTelegramQuizQuestion(question));
        const { data: generation, error: generationError } = await supabase.from("quiz_generations").insert({
          ...
          language: channel.settings.default_language || "en",
          source_type: knowledgeBase ? "knowledge_base" : "ai",
          quiz_data: { ...quiz, questions: normalizedQuestions, metadata: { ...(quiz.metadata || {}), language: channel.settings.default_language || "en", source_type: knowledgeBase ? "knowledge_base" : "ai", delivery_method: "telegram", telegram_chat_id: channel.telegram_channel_id, provider: resolved.provider, model: resolved.model } },
        }).select().single();
...
async function generateQuizForChannel(resolved: ResolvedAIProvider, aiSettings: AISettings, channel: Channel, topic: string, knowledgeBase: string, userSystemPrompt = ""): Promise<any> {
  ...
  const systemPrompt = composeTelePostSystemPrompt({
    ...
    outputRequirements: `You are an expert competitive-exam question setter. ${languageRule} Generate exactly ${count} standard government-competitive-exam MCQs about "${topic}". Each question must have exactly four plausible options and one correct answer. The correct answer MUST be dynamic and MUST NOT always be option A or B. Intentionally vary the correct option index across the quiz when the content permits, while keeping the answer factually correct. Never assume a fixed answer position. Keep questions under 120 characters, options under 80, explanations under 200, and output only JSON.`,
  });
  const userPrompt = `${knowledgeBase ? `Use only this channel knowledge base:\n${knowledgeBase}\n\n` : ""}Return exactly:\n{\n  "request_id": "${requestId}",\n  "topic": "${topic}",\n  "questions": [{"id": 1, "question": "string", "options": ["string", "string", "string", "string"], "correct_option_index": 0, "explanation": "string"}],\n  "metadata": {"generated_at": "${generatedAt}"}\n}\\nImportant: correct_option_index is zero-based and must point to the actual correct option in the exact options array. Do not use a fixed option position.`;
  let feedback = "";
  let lastError: Error | null = null;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const text = await chatCompletion({ resolved, messages: [{ role: "system", content: `${systemPrompt}${feedback ? `\nPrevious output failed: ${feedback}` : ""}` }, { role: "user", content: userPrompt }], temperature: Math.min(aiSettings.temperature ?? 0.7, 0.6), maxTokens: 4096, timeoutMs: 90000, appTitle: "TelePost Auto Quiz", settings: aiSettings });
      ...
      const invalid = quiz.questions.find((question: any) => {
        return !question?.question || !Array.isArray(question.options) || question.options.length !== 4 || !Number.isInteger(question.correct_option_index) || question.correct_option_index < 0 || question.correct_option_index > 3;
      });
...
async function sendQuizToTelegram(botToken: string, chatId: string, quiz: any): Promise<void> {
  ...
  const safeTruncate = (value: string, limit: number) => { const chars = Array.from(value || ""); return chars.length <= limit ? value : chars.slice(0, limit - 3).join("") + "..."; };
  ...
  for (let index = 0; index < quiz.questions.length; index++) {
    const normalizedQuestion = normalizeTelegramQuizQuestion(quiz.questions[index]);
    const response = await request(`${baseUrl}/sendPoll`, { chat_id: normalizedChatId, question: safeTruncate(`Q${index + 1}. ${normalizedQuestion.question}`, 290), options: normalizedQuestion.options.map((option) => safeTruncate(option, 95)), type: "quiz", correct_option_id: normalizedQuestion.correct_option_index, explanation: safeTruncate(normalizedQuestion.explanation || "Correct answer explanation", 190), is_anonymous: true });
