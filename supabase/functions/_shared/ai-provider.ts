...
export function cloudflareChatUrl(accountId: string, model = CLOUDFLARE_DEFAULT_MODEL): string {
  if (!model.startsWith('@cf/')) throw new Error(`Invalid Cloudflare Workers AI model ID: ${model}. Model IDs must start with @cf/.`);
...
function isUnknownModelError(status: number, body: string): boolean {
  ...
  return lowered.includes('no endpoints found') || lowered.includes('not a valid model') || lowered.includes('no allowed providers') || lowered.includes('model not found');
}
...
async function requestWithRetry(url: string, init: RequestInit, timeoutMs: number, maxAttempts = 3): Promise<Response> {
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      const response = await fetchWithTimeout(url, init, timeoutMs);
      if (response.ok || !isTransientStatus(response.status) || attempt === maxAttempts - 1) return response;
      const retryAfter = Number(response.headers.get('Retry-After'));
      const delayMs = Number.isFinite(retryAfter) ? Math.min(retryAfter * 1000, 15000) : Math.min(1000 * 2 ** attempt, 8000);
...
export async function chatCompletion(args: {
  ...
  if (resolved.provider === 'cloudflare') {
    ...
    for (const modelToTry of candidateModels) {
      const response = await requestWithRetry(cloudflareChatUrl(resolved.accountId!, modelToTry), {
        method: 'POST',
      ...
      if (!isUnknownModelError(response.status, body)) throw providerError(resolved.provider, response.status, body);
