...
serve(async (req: Request) => {
  ...
  try {
    ...
    const requestedSystemPrompt = typeof body.systemPrompt === 'string' ? body.systemPrompt : '';
    let language = typeof body.language === 'string' ? body.language : 'bn';
    ...
    const knowledgeBaseTopic = typeof body.knowledgeBaseTopic === 'object' && body.knowledgeBaseTopic !== null ? body.knowledgeBaseTopic as Record<string, string> : undefined;
    let userSystemPrompt = '';

    if (!topic && knowledgeBaseTopic?.topic_name) {
      topic = knowledgeBaseTopic.topic_name.trim();
    }
    if (knowledgeBaseTopic?.language && typeof body.language !== 'string') {
      language = knowledgeBaseTopic.language;
    }
    ...
    const { data: userPromptData } = await supabase.from('user_ai_system_prompts').select('system_prompt').eq('user_id', user.id).maybeSingle();
    userSystemPrompt = userPromptData?.system_prompt || '';

    if (knowledgeBaseTopic) {
      const parts = [`Topic: ${knowledgeBaseTopic.topic_name || topic}`];
      if (knowledgeBaseTopic.subject) parts.push(`Subject: ${knowledgeBaseTopic.subject}`);
      if (knowledgeBaseTopic.description) parts.push(`Description: ${knowledgeBaseTopic.description}`);
      if (knowledgeBaseTopic.exam) parts.push(`Target Exam: ${knowledgeBaseTopic.exam}`);
      if (knowledgeBaseTopic.grade) parts.push(`Grade: ${knowledgeBaseTopic.grade}`);
      knowledgeBaseContext = parts.join('\n');
    } else if (channelId && useChannelKnowledgeBase) {
    ...
    for (const batchSize of batches) {
      ...
      for (let attempt = 1; attempt <= ATTEMPTS_PER_BATCH; attempt++) {
        try {
          const baseSystemPrompt = composeTelePostSystemPrompt({
            platformInstructions: aiSettings.system_prompt,
            userSystemPrompt,
            featureInstructions: channelSystemPrompt || requestedSystemPrompt,
            knowledgeBaseInstructions: knowledgeBaseTopic?.ai_instructions || '',
            outputRequirements: `You are an expert Indian competitive-exam question setter.\n${languageRules[effectiveLanguage]}\nGenerate exactly ${wanted} high-quality MCQs about "${topic}" at ${difficulty} difficulty. Each question must have exactly four plausible options and one correct answer. Avoid all/none-of-the-above.\n\nCRITICAL OPTION RANDOMIZATION RULE: Distribute the correct answer dynamically and evenly across all four positions (option index 0, 1, 2, or 3). NEVER put the correct answer always in option 1 (index 0). Vary the correct option index dynamically across questions.\nDo NOT mention option letters (A, B, C, D) in the explanation text; explain the factual reason directly.\nKeep questions under 120 characters, options under 80, and explanations under 200. Focus on WBCS, WBP, SSC, Railway, Banking and other Indian government exams. Return JSON only, no markdown or commentary.\n\nBefore returning the JSON, silently verify that every question, every option and every explanation follows the requested language. If Bengali is requested, reject and rewrite any English sentence. বাংলা mode-এ English translation ব্যবহার করবেন না. ${lastReason ? `Previous attempt failed validation: ${lastReason}` : ''}`,
          });
          const userPrompt = `${knowledgeBaseContext ? `Use only this context:\n${knowledgeBaseContext}\n\n` : ''}${avoidList ? `Do not repeat these questions:\n${avoidList}\n\n` : ''}Create ${wanted} MCQs. The output must be valid JSON in exactly this shape:\n{"questions":[{"id":1,"question":"...","options":["...","...","...","..."],"correct_option_index":2,"explanation":"..."}]}\nNote: correct_option_index is the 0-based index (0, 1, 2, or 3) of the correct answer in the options array. Ensure correct answers are distributed randomly across all 4 option positions.\nFor Bengali mode, the visible text must be Bengali, not English translation.`;
